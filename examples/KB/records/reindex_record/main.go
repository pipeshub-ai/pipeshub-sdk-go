...
// Usage (from examples/):
//
//	go run ./KB/records/reindex_record <path-to-.env>
package main
...
func main() {
	if len(os.Args) < 2 {
		log.Fatal("usage: go run ./KB/records/reindex_record <path-to-.env>")
	}
	...
	res, err := sdk.KnowledgeBase.ReindexRecord(ctx, recordID, &components.ReindexRecordRequestBody{
		Force: pipeshub.Pointer(true),
	})
	if err != nil {
		log.Fatalf("reindex record: %v", err)
	}

	printJSON(res.ReIndexRecordResponseSchema)
}
