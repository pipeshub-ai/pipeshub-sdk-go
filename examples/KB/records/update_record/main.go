...
// Usage (from examples/):
//
//	go run ./KB/records/update_record <path-to-.env>
package main
...
func main() {
	if len(os.Args) < 2 {
		log.Fatal("usage: go run ./KB/records/update_record <path-to-.env>")
	}
	...
	res, err := sdk.KnowledgeBase.UpdateRecord(ctx, recordID, &operations.UpdateRecordRequestBody{
		RecordName: pipeshub.Pointer("Updated record name"),
	})
	if err != nil {
		log.Fatalf("update record: %v", err)
	}

	printJSON(res.Object)
}
