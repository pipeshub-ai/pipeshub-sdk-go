...
// Usage (from examples/):
//
//	go run ./KB/folder/delete_folder <path-to-.env>
package main
...
func main() {
	if len(os.Args) < 2 {
		log.Fatal("usage: go run ./KB/folder/delete_folder <path-to-.env>")
	}
	...
	res, err := sdk.KnowledgeBase.DeleteFolder(ctx, kbID, folderID)
	if err != nil {
		log.Fatalf("delete folder: %v", err)
	}

	printJSON(res.FolderDeleteResponseSchema)
}
