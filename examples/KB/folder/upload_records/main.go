// Upload a file into a folder (streaming the upload events).
//
...
// Usage (from examples/):
//
//	go run ./KB/folder/upload_records <path-to-.env>
package main
...
func main() {
	if len(os.Args) < 2 {
		log.Fatal("usage: go run ./KB/folder/upload_records <path-to-.env>")
	}
